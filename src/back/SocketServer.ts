import { BackState, OpenExternalFunc } from "@back/types";
import { BackIn, BackInTemplate, BackOut, BackOutTemplate, BackRes, BackResTemplate, ShowMessageBoxFunc } from "@shared/back/types";
...
export class SocketServer {
   ...
   */
    public showMessageBoxFactory(state: BackState, client?: BackClient): ShowMessageBoxFunc {
        if (client === undefined) {
        ...
        return async (options) => {
            if (client) {
                return state.socketServer.request(client, BackOut.OPEN_DIALOG, options);
            }
            return -1;
        };
    }
    ...
    // Wrap in context object so it can be mutated by middleware
        ...
        try {
            ...
            if (res.type !== BackOut.LOG_ENTRY_ADDED) {
                log("Launcher", "Error in middleware - Type: " + BackOut[type]);
            }
        }
        ...
        if (out) {
            if ("type" in data && "result" in out) {
                ...
                try {
                    ...
                    if (res.type !== BackOut.LOG_ENTRY_ADDED) {
                        log("Launcher", "Error in middleware - Type: " + BackRes[res.type]);
                    }
                }
            }
